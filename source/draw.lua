...
function ObjectiveEsp()
    ...
    for i = 1, #targets do
        ...
        if GetTagValue(body, "target") ~= "cleared" and GetTagValue(body, "target") ~= "disabled" then
            ...
            if dist > 2 then
                ...
                if optional then
                    if drawOptional then
                        ...
                        UiColor(optionalColor.red, optionalColor.green, optionalColor.blue,
                                optionalColor.alpha)
                    ...
                    UiColor(objectiveColor.red, objectiveColor.green, objectiveColor.blue,
                            objectiveColor.alpha)
                    UiText("Target", true)
                    UiText(math.floor(dist) .. "m")
                end
                UiPop()
            ...
            if optional then
                if drawOptional then
                    DrawBodyOutline(body, optionalColor.red, optionalColor.green,
                                    optionalColor.blue, optionalColor.alpha)
                end
            else
                DrawBodyOutline(body, objectiveColor.red, objectiveColor.green, objectiveColor.blue,
                                objectiveColor.alpha)
            end
        end
    end
